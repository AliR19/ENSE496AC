...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for capsule in currentGameState.getCapsules():
          if (manhattanDistance(capsule, newPos) < 1):
            capsuleSuccessor = 500 + 10*sum(newScaredTimes)

        return successorGameState.getScore() - 5*min(foodDist) + foodSuccessor + capsuleSuccessor
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def expValue(self, gameState, currentDepth, agentIndex):
      ...
      for action in gameState.getLegalActions(agentIndex):
        if agentIndex == gameState.getNumAgents()-1:
            expValue += probability * self.getValue(gameState.generateSuccessor(agentIndex, action), currentDepth+1, 0)
        else:
            expValue += probability * self.getValue(gameState.generateSuccessor(agentIndex, action), currentDepth, agentIndex+1)
      return expValue
...
def betterEvaluationFunction(currentGameState):
    ...
    foodDist = []
    for food in list(newFood.asList()):
        foodDist.append(float(manhattanDistance(food, newPos)))
    
    capsuleFactor = -50 * len(currentGameState.getCapsules()) + 0.5*sum(newScaredTimes)
    foodFactor = -10* min(foodDist)
    ghostFactor = min(ghostDist)

    return (currentGameState.getScore() + foodFactor + capsuleFactor + ghostFactor)  * (1.0 / float(currentGameState.getNumFood())) 
